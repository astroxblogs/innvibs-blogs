...
const BlogDetail = () => {
    ...
    return (
        <article className="max-w-4xl mx-auto bg-white dark:bg-gray-900 rounded-lg shadow-xl p-4 sm:p-6 md:p-8 mt-4 md:mt-8 mb-8"> {/* Adjusted top/bottom margin */}
            <img
                src={coverImage}
                alt={cleanAltTitle}
                className="w-full h-auto max-h-[300px] sm:max-h-[400px] object-cover rounded-lg mb-4 md:mb-6 bg-gray-200" // Adjusted max-height for mobile, reduced bottom margin
                loading="lazy"
            />

            <h1 className="text-2xl sm:text-3xl md:text-5xl font-semibold mb-2 md:mb-3 text-gray-900 dark:text-white leading-tight" style={{ fontFamily: 'Arial, sans-serif' }}>{displayTitle}</h1> {/* Adjusted font sizes, reduced bottom margin, added leading-tight */}
            <div className="flex flex-wrap gap-x-3 gap-y-1 items-center text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-6 md:mb-8"> {/* Adjusted gap, font size, bottom margin */}
            ...
            <div
                className="prose prose-base sm:prose-lg lg:prose-xl dark:prose-invert max-w-none mb-6 md:mb-8
                           prose-img:rounded-xl prose-img:max-h-[400px] prose-img:mx-auto" // Adjusted prose max-height for images
                dangerouslySetInnerHTML={{ __html: cleanContentHtml }}
            />

            <div className="flex flex-wrap gap-2 mb-6 md:mb-8"> {/* Adjusted bottom margin */}
                {blog.tags?.map((tag) => (
